...
| 10  | ESLint／Prettier／EditorConfig                         | **已完成**       | ESLint 8 + `@typescript-eslint` 5、`eslint-config-prettier`；`lint`／`format`／`format:check`；`.editorconfig`、`.prettierrc.json` |
| 11  | `panel.ts` 手寫 `chrome` 與 `@types/chrome` 收斂       | **已完成**       | 已無手寫 `declare const chrome`；依 `tsconfig` 的 `types: ["chrome"]`；`extensionChrome` 附 JSDoc                                  |
...
| `npm run lint:fix`     | ESLint 自動修正可修項目                                                                              |
| `npm run format`       | Prettier 寫入 TS、`scripts/*.mjs`、根目錄 json／md、`.eslintrc.cjs`、`docs`、`AGENT`／`ARCHITECTURE` |
| `npm run format:check` | Prettier 僅檢查、不寫檔                                                                              |
...
1. `npm run typecheck`
2. `npm run lint` 與 `npm run format:check`（建議）
3. `npm run build`
4. `AGENT.md` 手動流程（E 節）重點抽測；若使用 `build:prod`，建議完整跑一輪。
